...
static uint16_t abs_wrapper_16(uint16_t i){
    return abs(i);
}
...
void Thumb::SubstrateHookFunctionThumb(SubstrateProcessRef process, void *symbol, void *replace, void **result){
    ...
    if (
        ...
        arm[1] = (uint32_t)((uintptr_t)replace);
    ...
    for (unsigned offset(0); offset != used / sizeof(uint16_t); ++offset) {
        if (T$pcrel$ldr(backup[offset])) {
            ...
            *--trailer = ((static_cast<uint32_t>((uintptr_t)area + offset) + 4) & ~0x2) + bits.immediate * 4;
            ...
            *--trailer = static_cast<uint32_t>((uintptr_t)area + offset) + 4 + jump;
            ...
            *--trailer = T$msr_apsr_nzcvqg_rn(rt);
            ...
            // XXX: this code "works", but is "wrong": the mechanism is more complex than this
            *--trailer = ((static_cast<uint32_t>((uintptr_t)area + offset) + 4) & ~0x2) + (bits.u == 0 ? -exts.immediate : exts.immediate);
            ...
            *--trailer = static_cast<uint32_t>((uintptr_t)area + offset) + 4;
    ...
    transfer[1] = static_cast<uint32_t>((uintptr_t)area + used / sizeof(uint16_t)) + 1;
    ...
    *result = reinterpret_cast<uint8_t *>(buffer + pad) + 1;
...

    {
        ...
        arm[0] = A$ldr_rd_$rn_im$(A$pc, A$pc, 4 - 8);
        arm[1] = static_cast<uint32_t>((uintptr_t)replace);
